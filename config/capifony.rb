...
set :assets_children,   %w(css images)
...
namespace :deploy do
  ...
  task :finalize_update, :except => { :no_release => true } do
    ...
    run "cd #{latest_release} && #{php_path} #{latest_release}/symfony plugin:publish-assets"

    # permission
    run "cd #{latest_release} && #{php_path} #{latest_release}/symfony project:permissions"
...
namespace :symfony do
  def run_task task
    run "cd #{current_path} && #{php_path} #{current_path}/symfony #{task}"
  end
end
